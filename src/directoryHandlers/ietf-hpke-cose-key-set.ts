...
export async function handler(req: Request, env: Bindings): Promise<Response> {
  ...
  const keyToCoseKeyDecoded = async (key: R2Object) => {
    ...
    publicKeyCoseKeyMap.set(1, 3); // RSA https://www.rfc-editor.org/rfc/rfc8230.html
    publicKeyCoseKeyMap.set(2, new Uint8Array([Number.parseInt(metadata.tokenKeyID)]));
    publicKeyCoseKeyMap.set(3, -38); // RSASSA-PSS SHA-384 https://www.rfc-editor.org/rfc/rfc8230.html
    publicKeyCoseKeyMap.set(4, [2]); // verify https://datatracker.ietf.org/doc/html/rfc8152#section-7.1
    publicKeyCoseKeyMap.set(-1, b64Tou8(b64URLtoB64(publicKeyJwk.n!)));
    publicKeyCoseKeyMap.set(-2, b64Tou8(b64URLtoB64(publicKeyJwk.e!)));
